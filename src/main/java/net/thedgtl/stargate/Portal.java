...
public final class Portal {
    ...
    public void close(boolean force) {
        ...
        if (isAlwaysOn() && !force) {
            return; // Only close always-open if forced
        }
    ...
    public Location getExit(Location traveller) {
        ...
        if (gate.getExit() != null) {
            ...
            stargate.getLogger().log(Level.WARNING, "Missing destination point in .gate file " + gate.getFilename());
        }
    ...
    public final void drawSign() {
        ...
        if (sMat != Material.SIGN && sMat != Material.WALL_SIGN && sMat != Material.SIGN_POST) {
            stargate.getLogger().warning("Sign block is not a Sign object");
        ...
        if (!isActive()) {
            ...
            if (isBungee()) {
                ...
                if ((index == max) && (max > 1) && (++done <= 3)) {
                    if (stargate.getEconomyHandler().isEnabled() && stargate.getEconomyHandler().freeGatesGreen) {
    ...
    public static Portal createPortal(Stargate stargate, SignChangeEvent event, Player player) {
        ...
        Blox button = null;
        Portal portal = null;
        portal = new Portal(stargate, topleft, modX, modZ, rotX, id, button, destName, name, false, network, gate, player.getName(), hidden, alwaysOn, priv, free, backwards, show, noNetwork, random, bungee);
    ...
    public static void saveAllGates(Stargate stargate, World world) {
        ...
        try {
            ...
            stargate.getLogger().log(Level.SEVERE, "Exception while writing stargates to " + loc + ": " + e);
        }
    }
    ...
    public static void loadAllGates(Stargate stargate, World world) {
        ...
        if (db.exists()) {
            ...
            try {
                ...
                while (scanner.hasNextLine()) {
                    ...
                    if (split.length < 8) {
                        stargate.getLogger().info("Invalid line - " + l);
                        continue;
                    }
                    String name = split[0];
                    Blox sign = new Blox(world, split[1]);
                    if (!(sign.getBlock().getState() instanceof Sign)) {
                        stargate.getLogger().info("Sign on line " + l + " doesn't exist. BlockType = " + sign.getBlock().getType());
                        continue;
                    }
                    ...
                    if (gate == null) {
                        stargate.getLogger().info("Gate layout on line " + l + " does not exist [" + split[7] + "]");
                        continue;
                    }
                    ...
                    Portal portal = new Portal(stargate, topLeft, modX, modZ, rotX, sign, button, dest, name, false, network, gate, owner, hidden, alwaysOn, priv, free, backwards, show, noNetwork, random, bungee);
                    portal.register();
                    portal.close(true);
                }
                scanner.close();
                ...
                for (Iterator<Portal> iter = allPortals.iterator(); iter.hasNext();) {
                    ...
                    if (!portal.wasVerified()) {
                        if (!portal.isVerified() || !portal.checkIntegrity()) {
                            ...
                            iter.remove();
                            stargate.getLogger().info("Destroying stargate at " + portal.toString());
                            continue;
                        } else {
                            portal.drawSign();
                            portalCount++;
                ...
                stargate.getLogger().info("{" + world.getName() + "} Loaded " + portalCount + " stargates with " + OpenCount + " set as always-on");
            } catch (Exception e) {
                stargate.getLogger().log(Level.SEVERE, "Exception while reading stargates from " + db.getName() + ": " + l);
                e.printStackTrace();
            }
        } else {
            stargate.getLogger().info("{" + world.getName() + "} No stargates for world ");
        }
    }

    public static void closeAllGates(Stargate stargate) {
        stargate.getLogger().info("Closing all stargates.");
